	...
	module.exports = new Class( {
		...
		, get prefetchPercent(){ return ( this.free + this.requested ) / ( this.queue.length + this.busy ) * 100; }
		...
		, onCloseResource: function( resource ){
			if ( resource.busy ) this.busy--;
				...
				if ( idx ) this.resources.splice( idx, 1 );
		...
		, requestResoure: function(){
			...
			if ( ( this.queue.length > 0 || ( this.prefetch > 0 && this.prefetchPercent < this.prefetch ) ) && ( this.max === 0 || ( this.total + this.request ) < this.max ) ){
				this.requested++;
